...
class OrderModify {
  public:
    ...
    OrderPointer ToOrderPointer(OrderType type) const {
        return std::make_shared<Order>(type, GetOrderId(), GetSide(),
...
class OrderBook {
  private:
    struct OrderEntry {
        ...
        OrderPointers::iterator location_; // Allowed 0(1) erase time of the node that has the pointer since we know where the location is in memory
    };
    ...
    bool CanMatch(Side side, Price price) const {
        if (side == Side::Buy) {
            if (asks_.empty())
                return false; // No ask to match with

            const auto &[bestAsk, _] = *asks_.begin();
            return price >= bestAsk;
        } else {
            if (bids_.empty())
                return false; // No bids to match with
...
public: 
    Trades AddTrade(OrderPointer order)
    {
        ...
        // FAK cancels it, if order can not be filled immediately
    ...
    void CancelTrade(OrderId orderId)
    {
        if (!orders_.empty() || !orders_.contains(orderId))
            return;
        
        // 0(1) erase since we store its location
